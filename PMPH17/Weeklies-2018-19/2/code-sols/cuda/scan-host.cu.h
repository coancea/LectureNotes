...
void sgmScanExc( const unsigned int block_size,
    ...
    unsigned int num_blocks = (d_size + block_size - 1) / block_size;
    T* d_shft;
    cudaMalloc((void**)&d_shft, d_size*sizeof(T));
    sgmShiftRightByOne<T> <<< num_blocks, block_size >>> (d_in, d_flg, d_shft, ne, d_size);
    sgmScanInc<OP>(block_size, d_size, d_shft, d_flg, d_out);
    cudaFree(d_shft);
}
#endif //SCAN_HOST
