...
/**
 * block_size denotes the size of a CUDA block.
 * mar_rows denotes the number of rows in the matrix; we assume that no row is empty.
 * vct_size is the length of the vector. 
 */
...
#if 1
    { // GPU execution    
        int   *flags_d, *mat_inds_d, *mat_shp_d, *mat_shp_sc_d;
        ...
        // ToDo: a) Fill in a meaningful implementation for
        ...
        // ToDo b) implement the four kernels called below in file `spMV-Mul-kernels.cu.h`
        { // Make flag array
            // 1. scan the shape array
            scanInc< Add<int> > ( block_size, tot_size, mat_shp_d, mat_shp_sc_d );
        ...
        { // Kernels
            ...
            //    result-vector index.
            sgmScanInc< Add<float> > ( block_size, tot_size, tmp_pairs, flags_d, tmp_scan );
