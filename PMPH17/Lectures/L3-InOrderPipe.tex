...
\date[Sept 2018]{September 2018 PMPH Lecture Notes}
...
{\bf Reduced Instruction Set Architecture (RISC):}\pause
\begin{itemize} 
...
\begin{scriptsize}
Complex instr fetched from {\tt InstrMemory} \& decoded:\pause
\begin{itemize}
...
\item Pipeline stages need to be very similar. Instruction execution good
        candidate \pause because goes through same stages: fetch, decode, execute, 
...
\item \emp{Main Problem}\pause is to handle efficiently data and control dependencies
...
\item \emp{Clean Pipeline Design}: \pause instr is recoded as it moves through pipeline,
...
What hazards can occur in the 5-Stage Pipeline?
\begin{itemize}
\item I1 updates register R1 at the trailing edge of clock C5. \pause \smallskip
...
\item[1] \emp{Forwarding is more complicated} \pause because float stores need the
...
\item[2] \emp{Hazard Detection Unit (HDU) is more complicated} \pause because both RAW and WAW
...
\item For example, branches are executed in EX1 and have penalty \pause 3.
\item Latency (of operation) of loads is \pause 3.
\item Latency of register-to-register instructions is \pause 1 (\# of EX stages-1).
\end{itemize}
\end{scriptsize}
\end{frame}
...
No Two Instructions of the Same Thread Are In The Pipeline in the Same Time.}
...
\begin{itemize}
\item Data/Control/Structural Hazards \pause cannot occur because of the \alert{assumption};\\
        Precise Exceptions \pause are respected because of the \alert{assumption}.  \alert{Problems are}:
    \begin{itemize}\pause
    \begin{scriptsize}
        ...
        \item[2] memory wall was getting larger and larger, 
