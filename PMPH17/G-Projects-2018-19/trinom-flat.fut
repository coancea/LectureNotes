...
let trinomialChunk [ycCount] [numAllOptions] [maxOptionsInChunk]
  ...
  let (firstthree, lastfour) = unzip <|
    map (\i -> if (i < 0)
               then ((1.0, 0, 1.0), (1.0, 1.0, -1, -1))
               else
                let option = options[i] -- unsafe
  ...
  let (Xs, ns, dts)        = unzip3 firstthree
  let (drs, Ms, jmaxs, ms) = unzip4 lastfour
  ...
  let ((_,alphass)) = loop ((Qs,alphass)) for i < n_maxInChunk do
      ...
      --                then zero
      --                else fwdHelper M dr dt (alphas[i]) Q m i imax jmax j
      --         ) (map (-m) (iota (2*m+1)))
                                ...
                                then 0.0
                                     ...
                                     then 0.0
                                          ...
                                          fwdHelper (Ms[sgm_ind]) (drs[sgm_ind]) (dts[sgm_ind]) 
                                                    (alphass[sgm_ind*seq_len+i]) (Qs,begind) 
  ...
  let (Calls) = loop (Calls) for ii < n_maxInChunk do  -- condition is ii < ns[sgm_ind]
      ...
      --                     then zero -- Call[j + m]
      --                     else bkwdHelper X M dr dt (alphas[i]) Call m i jmax j
      --              ) (map (-m) (iota (2*m+1)))
...
let formatOptions [numOptions]
  ...
  let m_min = reduce (i32.min) (i32.highest) ms

  let maxOptionsInChunk = w / m_min
