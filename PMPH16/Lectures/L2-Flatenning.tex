...
(attached in TeachingMaterial/AdditionalMaterial/ListHom-Flattening).
\bigskip
...
\begin{colorcode}[fontsize=\scriptsize]
\emp{-- 1. scan nested inside a map:}
...
\begin{colorcode}[fontsize=\scriptsize]
\emp{-- 4. replicate nested inside a map:}
let arr = \emp{[1,3,2]} in
map (\mymath{\backslash}n -> replicate(n, n)) arr 
\mymath{\equiv}
[ replicate(1,1)
, replicate(3,3)
, replicate(2,2) ] 
...
\emp{-- 7. if nested inside a filter}
let arr = [3, 4, 6, 7] in
map(\mymath{\backslash}x -> 
        if (odd x)
        then f x  -- assume f is *2
        else g x  -- assume g is -1
   ) arr 
-- \mymath{\Rightarrow} [6, 3, 5, 14]

\emp{-- 6. filter nested inside a map:}
map(\mymath{\backslash}row -> filter p row)

\end{colorcode}
\column{0.55\textwidth}
\begin{colorcode}[fontsize=\scriptsize]
\emph{--build the flag array and sgmScan it!}
inds = sgmScan\mymath{\myindu{exc}} (+) 0 arr
size = (last inds) + (last arr)
flag = write arr   -- \emp{[1,3,2]}
             inds  -- \emp{[0,1,4]}
             replicate(size, 0)
--           [1,3,0,0,2,0]
sgmScan\mymath{\myindu{inc}} (+) 0 flag
-- [1,3,3,3,2,2]

\emph{-- scatter o map o gather}
ais = zip arr (iota (length arr))
(ais',flg)=parFilter(\mymath{\backslash}(x,_)->p e) ais
(ais\mymath{\myindu{t}},ais\mymath{\myindu{f}}) = split flg[0] ais'
(arr\mymath{\myindu{t}},inds\mymath{\myindu{t}}) = unzip ais\mymath{\myindu{t}}
(arr\mymath{\myindu{f}},inds\mymath{\myindu{f}}) = unzip ais\mymath{\myindu{f}}
(arr\mymath{\myindu{then}},arr\mymath{\myindu{else}}) = (map f arr\mymath{\myindu{t}}, map g arr\mymath{\myindu{f}})
restmp = write inds\mymath{\myindu{t}} arr\mymath{\myindu{then}} [0,\mymath{\ldots},0]
result = write inds\mymath{\myindu{f}} arr\mymath{\myindu{else}} restmp
\emph{-- write filter in terms of map/scan}
\emph{-- the flatten each operation!}
...
\subsection{Flattening Prime-Number Computation}
