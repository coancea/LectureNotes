...
Finally we can build a GPU kernel out of it!
...
__global__ void matMultTiledKer(...) \{
  \alert{__shared__ T Ash[L,L], Bsh[L,L];}
  float tmp = 0.0
  int gidy = blockIdx.y*blockDim.y + 
             threadIdx.y // same for gidx
  int gidx = blockIdx.x*blockDim.x + threadIdx.x 
  DO kk = 1, N, L
    \alert{Ash[threadIdx.y,threadIdx.x] = A[gidy,kk+threadIdx.x]}
    \alert{Bsh[threadIdx.y,threadIdx.x] = B[threadIdx.y+kk,gidx]}
    \alert{DO k = 1, L}
       \alert{tmp += Ash[threadIdx.y][k] * Bsh[k][threadIdx.x]}
    ENDDO ENDDO 
    C[i,j] = tmp;
\}
\end{colorcode} 
...
A global memory access is amortized by (L-1) shared memory accesses.
