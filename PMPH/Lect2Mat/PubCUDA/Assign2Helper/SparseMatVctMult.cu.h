...
int SparseMatVctMult(int block_size, int mat_rows, int vct_size) {
    ...
    { // GPU execution    
        ...
        { // Kernels
            ...
            //    IMPLEMENT kernel `spMatVctMult_pairs' in ScanKernels.cu.h
            spMatVctMult_pairs<<< num_blocks, block_size >>>
                    (mat_inds_d, mat_vals_d, vct_d, tot_size, tmp_pairs);
            cudaThreadSynchronize();
 
            // 2. perform a segmented scan, such as the last element 
            //    of each segment has the value of the corresponding 
            //    result-vector index.  
            //    `tot_size' is the total number of non-zero matrix elements.
            //    `flags_d'  is the flag array denoting the start of each row 
            //    `tmp_pairs' is the array to be scanned over
            //    `tmp_scan'  is the result array (of this operation)
            //    ... fill in the blanks below ...
            //sgmScanInc< Add<float>, float > ( ..., ..., ..., ..., tmp_scan );

            // 3. scan inclusive on flags computes the indices (+1) in the result
            //    vector where each of the last element of a segment needs to be placed.
            //    These computed indices are placed in `tmp_inds' 
            //                      (the result array of this op)
            //    ... fill in the blanks below ...
            //scanInc< Add<int>, int > ( ..., ..., ..., tmp_inds );

            // 4. finally, place the last element of each segment in the
            //    corresponding index in the result vector
            //    IMPLEMENT kernel `write_lastSgmElem' in ScanKernels.cu.h
            write_lastSgmElem<<< num_blocks, block_size >>>
                        (tmp_scan, tmp_inds, flags_d, tot_size, res_vct_d);
            cudaThreadSynchronize();
        }
