...
void scanIncAdd(    unsigned int  block_size,
    ...
    scanIncAdd<OP,T>( block_size, num_blocks, d_rec_in, d_rec_out );
...
void scanExcAdd (   unsigned int  block_size,
    ...
    unsigned int num_blocks;
    T *d_out_tmp;
    cudaMalloc((void**)&d_out_tmp , d_size*sizeof(T));

    scanIncAdd<OP,T>( block_size, d_size, d_in, d_out_tmp );

    num_blocks = ( (d_size % block_size) == 0) ?
                    d_size / block_size     :
                    d_size / block_size + 1 ;
    shiftRightByOne<T><<< num_blocks, block_size >>>(d_out_tmp, d_out, 0.0, d_size);
}
...
void sgmScanIncAdd( unsigned int  block_size,
    ...
    sgmScanIncAdd<OP,T,OP_INT>( block_size, num_blocks, d_rec_in, f_rec_in, d_rec_out );
    ...
    scanIncKernel<OP_INT,int><<< num_blocks, block_size, flg_sh_size >>>(flags, f_inds, d_size);
