...
/**
 * if VERSION is set to 1 => tests matrix transposition
 *     i.e., Assignment 2, task 1
 * if VERSION is set to 2 => tests the original program
 *     that exhibits uncoalesced access,
 *     i.e., Assignment 2, task 2
 * if VERSION is set to 3 => tests the transfromed program
 *     in which memory coalescing has been achieved via
 *     transposition, i.e., Assignment 2, task 2
 */
#define VERSION 3


template<class T, int tile>
void transpose( float*             inp_d,  // device
                float*             out_d,  // device
   ...
   // tiled version of the transpose kernel
   matTransposeTiledKer<float,tile> <<< grid, block >>>
                       (inp_d, out_d, height, width);

   // naive version of the transpose kernel 
...
int main() {
   ...
   { // compute original program
    ...
    { // compute transformed program
        ...
        // FILL IN: what is the width/height of A?
        transpose<float, TILE>( d_A, d_Atr, ..., ... );

        transfProg<<<grid,block>>>(d_Atr, d_Btr, num_thds);
        cudaThreadSynchronize();

        // FILL IN: what is the width/height of B?
        transpose<float, TILE>( d_Btr, d_B, ..., ... );
