...
int MsspProblem(int block_size, int inp_size) {
    ...
    { // KERNELS
        ...
        //    IMPLEMENT THE KERNEL in ScanKernels.cu.h
        msspTrivialMap<<< num_blocks, block_size >>>(inp_d, inp_lift, inp_size);
        cudaThreadSynchronize();
    
        // 2. apply scan inclusive with the given operator, i.e.,
        //    Fill in the blanks the ``MsspOp::apply'' in class MsspOP  
        //    in ScanKernels.cu.h and call scanInc from ScanHost.cu.h  
        //    decomment and fill in the line below
        //scanInc< MsspOp,MyInt4 > ( ..., ..., ..., res_d );
        cudaThreadSynchronize();
    }
    MyInt4 res_h(0,0,0,0);
    // 3. copy back only the last element of the res_d array (of size sizeof(MyInt4))
    //        fill in the position of the last element below (and decomment)
    //cudaMemcpy(&res_h, ..., sizeof(MyInt4), cudaMemcpyDeviceToHost);
